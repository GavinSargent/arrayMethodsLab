...
// CODE HERE
const myStrongest  = monstersInYourPocket.filter((element, index, array) => {
...
const totalPrice = orders.map((element, index, array)=> {
...
const bobsBill = purchases.filter((element)=>{
  return element.owner === "Bob"
})
const bobsTotalPrice = bobsBill.reduce((bobsAccPrice, bobsCurrentTotalPrice) => {
  return bobsAccPrice + bobsCurrentTotalPrice.price
},0)

console.log(bobsTotalPrice)